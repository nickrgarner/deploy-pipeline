  ...
  await new Promise(async (resolve) => {
    for (var i = 1; i <= iterations; i++) {
      ...
      await child.execSync(
        `cd /tmp/iTrust2-v8 && mysql -u root --password=${process.env.MYSQL_PASS} -e 'DROP DATABASE IF EXISTS iTrust2'`,
      );
      ...
      try {
        await new Promise((resolve, reject) => {
          const spawn = child.exec(
            `cd /tmp/iTrust2-v8/iTrust2 && mvn clean test -l maven.log`,
          );
          var logContents = fs.readFileSync(
            "/tmp/iTrust2-v8/iTrust2/maven.log",
            "utf-8",
          );
